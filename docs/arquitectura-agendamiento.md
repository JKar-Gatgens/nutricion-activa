...
## 7. HU-05 — Notificaciones por correo (implementado, 2026-07-17)

Al confirmarse una reserva, `ReservaCitaService.reservar(...)` publica `CitaReservadaEvent` (solo el `id` de la cita) como último paso dentro de su propia transacción. `NotificacionCitaListener` reacciona con `@Async` + `@TransactionalEventListener(phase = AFTER_COMMIT)`: el correo sale en otro hilo y solo si la transacción confirmó — si `reservar(...)` termina en excepción (slot ocupado, servicio no agendable), el evento nunca se publica y no sale ningún correo. Cada uno de los dos envíos (cliente, nutricionista) queda en su propio `try/catch`: un fallo se loguea con el id de la cita y nunca se propaga, y el fallo de uno no cancela el intento del otro. `@EnableAsync` vive en `AsyncConfig` (`src/main/java/com/nutricionactiva/config/`), junto con el registro de `MailProperties` — deliberadamente separado de `GlobalModelAttributes` (que registra `WhatsappProperties`/`SitioProperties`) porque `MailProperties` no la consume ninguna vista, así que no necesita estar disponible en los slice tests `@WebMvcTest`.

**Nota de implementación — autoconfiguración de mail (a diferencia de Flyway, SÍ existe):** antes de implementar se verificó como supuesto abierto si `spring-boot-starter-mail` traía autoconfiguración en Spring Boot 4.1.0, dado que Flyway no la trae (sección 5). Se confirmó inspeccionando el jar resuelto (`spring-boot-mail-4.1.0.jar`) que **sí** trae `MailSenderAutoConfiguration` completa, solo que reubicada al paquete `org.springframework.boot.mail.autoconfigure` (antes `org.springframework.boot.autoconfigure.mail` en Boot 3) — parte de la misma modularización que separó `spring-boot-webmvc`, `spring-boot-data-jpa`, etc. en artefactos propios. No hizo falta ningún `@Bean JavaMailSenderImpl` manual: basta con la dependencia + `spring.mail.*` en `application.properties`.

**Nota de implementación — efecto secundario en `ReservaCitaServiceConcurrenciaTest`:** ese test (paso 5) no mockeaba `JavaMailSender`. Al agregar el evento post-commit, su única reserva exitosa empezó a disparar -en el hilo `@Async`- un intento de conexión SMTP real contra `smtp.gmail.com` (fallaba rápido por falta de credenciales, gracias al try/catch del listener nunca rompió el test, pero seguía siendo una llamada de red real e innecesaria en un test que antes corría 100% contra Testcontainers sin salir a internet). Se corrigió agregando `@MockitoBean JavaMailSender` a ese test, igual que en los tests nuevos de HU-05.
